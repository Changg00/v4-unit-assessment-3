...
class Character {
  ...
  getInfo() {
    let phrase1 = `This is a ${this.type} character named ${this.name}.`
    return phrase1;
  }
}
...
class NPC extends Character {
  ...
  dialogue() {
    let phrase2 = `${this.name}: ${this.phrase}`
    return phrase2;
  }
}
...
class Player extends Character {
  ...
  defend(amount) {
    ...
    if (this.healthLevel >0) {
      return {attackStrength:amount, remainingHealth:this.healthLevel, message: `${this.name} is still in the fight!`}
...
class Hero extends Player {
  ...
  useSuperPower(index) {
  let zophir = this.superPowers[index]
  return `${this.name} used ${zophir}!`
  }
}
...
fireSpitter.addSuperPower('walking google')
fireSpitter.addSuperPower('mind control')
